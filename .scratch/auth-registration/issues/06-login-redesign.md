...
- [~] 聚焦收缩：初版实现了 `LoginHero collapsed` 高度→0 + 淡出让位表单——**后按产品反馈取消**：聚焦输入时不再整页上移 / hero 收缩，页面保持沉稳静态（同时去掉 `Screen avoidKeyboard`，避免键盘引发的上移）。`LoginHero` 回退为纯品牌动效、无 `collapsed` prop。
- [x] 布局改为 `flexGrow` 撑满视口 + 弹性间隔，把「其他登录方式」下沉至底部、与底边留合理距离（`form flex:1 + paddingBottom:30`，`spacer flexGrow:1`）；矮屏自然滚动。
