...
export function conversationRoutes({ db }: AppDeps) {
  ...
  app.post('/', zValidator('json', CreateConversationInputSchema), (c) => {
    ...
    if (system === 'bazi') {
      ...
      if (!profile) {
        return c.json({ error: { code: 'birth_required', message: '八字对话需先完善生辰' } }, 409);
      }
    }
    const id = createId();
    ...
    if (!row) return c.json({ error: { code: 'internal', message: '创建对话失败' } }, 500);
  ...
  app.put('/:id/favorite', zValidator('json', SetFavoriteInputSchema), (c) => {
    const userId = c.get('userId');
    ...
    return c.json(toConversation(updated ?? row));
  });
