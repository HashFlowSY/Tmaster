...
export function authMiddleware(db: Db, secret: string, idleMs: number) {
  return createMiddleware<{ Variables: AuthVariables }>(async (c, next) => {
    ...
    if (!header?.startsWith('Bearer ')) {
      return c.json({ error: { code: 'unauthorized', message: '缺少凭证' } }, 401);
    }
    let payload: { sub: string; sid: string };
    try {
      payload = await verifyToken(header.slice(7), secret);
    } catch {
      return c.json({ error: { code: 'unauthorized', message: '凭证无效' } }, 401);
    }
    const session = touchSession(db, payload.sid, idleMs);
    if (!session || session.userId !== payload.sub) {
      return c.json({ error: { code: 'unauthorized', message: '会话已过期，请重新登录' } }, 401);
    }
